...
// Ensure ensureRawEventsSheetExists or somewhere in buildDashboardSheets we fetch Product Cart Analysis
const fetchRowsStr = `const rows = await fetchRawEventRows(s);
  console.log(\`[RAW_EVENTS_ROWS_FOUND] \${rows.length}\`);
  const aggregation = buildAggregations(rows);`;

const cartAndProductsCode = `
  console.log('[SYNC] Fetching Product Cart Analysis for reconciliation...');
  ...
  const IGNORED_ROWS = new Set([
    ...
    'total active carts', 'total abandoned carts', 'fastest converting product', 'top products', 'low conversion products'
  ]);

  let cartOriginalNames = new Map();
  try {
    const cartRes = await s.spreadsheets.values.get({ spreadsheetId: spreadsheet.data.spreadsheetId, range: "'Product Cart Analysis'!A2:M" });
    const cartRows = cartRes.data.values || [];
    cartRows.forEach(r => { 
      const pName = String(r[1]).trim();
      if (pName && pName !== '' && !IGNORED_ROWS.has(pName.toLowerCase())) {
        const norm = normalizeProductName(pName);
        if (norm) cartOriginalNames.set(norm, pName);
      }
    });
  } catch (e) {
    console.error('Failed to fetch Product Cart Analysis', e.message);
  }
  ...
  // Inject missing products from Cart Analysis into aggregation
  cartOriginalNames.forEach((origName, norm) => {
    let found = false;
    for (let p of aggregation.productRows) {
      if (normalizeProductName(p.productName) === norm) { found = true; break; }
    }
    if (!found) {
      aggregation.productRows.push({
  ...
  // Flag products in backend not in cart
  aggregation.productRows.forEach(p => {
    const norm = normalizeProductName(p.productName);
    if (!cartOriginalNames.has(norm) && !IGNORED_ROWS.has(p.productName.toLowerCase())) {
      p.missingInCart = true;
    }
  });
`;

if (!code.includes('Product Cart Analysis for reconciliation')) {
  code = code.replace(fetchRowsStr, fetchRowsStr + '\n' + cartAndProductsCode);
}
...
const newProdValsMap = `    ...aggregation.productRows.slice(0, 500).map(p => {
      ...
      if (price === null) console.warn('[MISSING_PRICE]', p.productName);
      ...
      return [
        pName, pName, priceVal, viewsFormula, vValFormula, ppvFormula, ppValFormula,
        cpvFormula, cpValFormula, ptcFormula, cvValFormula, ctcFormula, ctcValFormula,
        ccpvFormula, potRevFormula, p.purchases || 0, actRevFormula,
...
if (!code.includes('Product View Value')) {
  code = code.replace(regexOldProdVals, newProdValsStart + '\\n' + newProdValsMap);
}

// Replace Executive Dashboard KPIs
...
console.log('googleSheetsService.js completely rebuilt and patched!');
