...
class AnalyticsService {
    ...
    private sessionUtm: { source: string; medium: string; campaign: string; content: string; term: string; gclid: string; fbclid: string; };
    private lastPageView: { page: string, timestamp: number, url: string, title: string } | null = null;
    ...
    private resolveTrafficSource() {
        if (typeof window === 'undefined') {
            return {
                ...
                sessionUtm: { source: '', medium: '', campaign: '', content: '', term: '', gclid: '', fbclid: '' }
            };
        }
        ...
        if (currentUtm.gclid) sessionStorage.setItem('kottravai_session_gclid', currentUtm.gclid);
        if (currentUtm.fbclid) sessionStorage.setItem('kottravai_session_fbclid', currentUtm.fbclid);
        ...
        const sessionUtm = {
            ...
            gclid: sessionStorage.getItem('kottravai_session_gclid') || '',
            fbclid: sessionStorage.getItem('kottravai_session_fbclid') || ''
        };
    ...
    private createPayload(eventType: string, page: string, metadata: AnalyticsMetadata = {}): AnalyticsPayload {
        ...
        const userId = typeof localStorage !== 'undefined' ? localStorage.getItem('kottravai_user_id') : null;
        ...
        const payload: AnalyticsPayload = {
            ...
            user_id: userId,
            is_logged_in: !!userId,
            gclid: normalizeValue(this.sessionUtm.gclid),
            fbclid: normalizeValue(this.sessionUtm.fbclid),
    ...
    public sendPageLeave() {
        if (!this.lastPageView) return;
        const duration_seconds = Math.floor((Date.now() - this.lastPageView.timestamp) / 1000);
        
        if (duration_seconds > 0) {
            const payload = this.createPayload('page_leave', this.lastPageView.page, { duration_seconds });
            payload.page_url = this.lastPageView.url;
            payload.page_title = this.lastPageView.title;
            payload.duration_seconds = duration_seconds;
            // Send synchronously if beforeunload, but fetch keepalive handles this asynchronously in Modern browsers
